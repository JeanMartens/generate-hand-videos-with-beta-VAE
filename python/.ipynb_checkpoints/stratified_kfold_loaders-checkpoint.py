...
def kfold_loaders(metadata,all_images, normalise_transform = None, batch_size_train = 64, batch_size_valid = 64, num_splits=5, random_state=42):
    ...
    for train_indices, valid_indices in kf.split(metadata):
        ...
        train_dataset = PreprocessedDataset(train_metadata,all_images, normalise_transform, train=True)
        valid_dataset = PreprocessedDataset(valid_metadata,all_images, normalise_transform, train=False)
    ...
    return train_loaders, valid_loaders, list(kf.split(metadata))
